#ifndef __BEX_IO_HTTP_ESCAPE_STRING_HPP__
#define __BEX_IO_HTTP_ESCAPE_STRING_HPP__
...
inline std::string unescape_path_copy(const std::string& in)
{
    std::string out;
    if (!unescape_path(in, out))
        return std::string();
...
#endif //__BEX_IO_HTTP_ESCAPE_STRING_HPP__