...
namespace Bex { namespace bexio
{
    ...
    public:
        ...
        virtual void shutdown()
        {
            ...
            shutdowning_.set();
            on_error(make_error_code(bee::initiative_shutdown));
            ...
            if (opts_->ssl_opts)
            {
                ...
                timeout_handler.async_wait(BEX_IO_BIND(&this_type::on_async_shutdown, this, make_error_code(bee::handshake_overtime), shared_this()));
        ...
        virtual void terminate()
        {
            ...
            on_error(make_error_code(bee::initiative_terminate));
            error_code ec;
    ...
    private:
        ...
        void on_async_receive(error_code ec, std::size_t bytes, shared_ptr<this_type>)
        {
            ...
            if (ec) 
            {
                ...
                {
                    shutdowning_.set();
                    on_error(make_error_code(bee::passive_shutdown));
                }
                else
                    on_error(ec);
        ...
        void on_shutdown_overtime(error_code const& ec, shared_ptr<this_type>)
        {
            ...
            shutdown_dt_.reset();
            ec_ = make_error_code(bee::shutdown_overtime);   // 强制修改断开错误原因
            terminate();
        }
    ...
    private:
        ...
        bool on_sendbuffer_overflow()
        {
            ...
            {
                on_error(make_error_code(bee::sendbuffer_overflow));
                return false;
            }
        ...
        bool on_receivebuffer_overflow()
        {
            ...
            {
                on_error(make_error_code(bee::receivebuffer_overflow));
                return false;
            }
